...
artistControllers.controller('DetailsController',function($scope,$routeParams){
  ...
  $scope.whichUser = $routeParams.userId;
    ...
    $scope.prevUser = Number($routeParams.userId) - 1;
      ...
      $scope.prevUser = $scope.artists.length - 1;
    };

    if ($routeParams.userId < $scope.artists.length - 1) {
